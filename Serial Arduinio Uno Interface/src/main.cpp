...
void setup()
{
  ...
  Serial.println("Respiratory Monitoring V4.1");
...
float fake_pressure = 8;
float fake_airflow = 2;
...
void loop()
{
  ...
  // if (pressure_L > 1)
  // {
  //   pressure_L = pressure_L * fake_pressure * 2;
  // }

  //average out when needed and calculate direction
  // averageAndDirection(pressure_L, prev_pressure_L, direction_L);
  averageAndDirection(pressure_R, prev_pressure_R, direction_R);

  //calculate volume
  // tidalVolume(airflow_L, volume_L, direction_L, 20);
  ...
  Serial.print(airflow_L * direction_R);
